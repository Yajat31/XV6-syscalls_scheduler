...
int random(void)
{
  ...
  x ^= x << 5;
  xorshift_state = x;
  return (int)(x & 0x7FFFFFFF); // Return a non-negative integer
}
...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    // Count total tickets and collect runnable processes
    for(p = proc; p < &proc[NPROC]; p++) {
      acquire(&p->lock);
      if(p->state == RUNNABLE) {
        total_tickets += p->tickets;
        runnable_procs[num_runnable++] = p;
      }
      release(&p->lock);
    }

    if(total_tickets == 0 || num_runnable == 0) continue;
    ...
    for(int i = 0; i < num_runnable; i++) {
      p = runnable_procs[i];
      if(p->tickets == winner->tickets && p->ctime < winner->ctime){
        winner = p;
      }
    }
    ...
    #ifdef SCHEDULER_MLFQ
    int found = 0;
    static int last_proc_index = 0;  // For round-robin in lowest priority

    // Find the highest priority runnable process
    for(int priority = 0; priority < 4; priority++) {
      if(priority < 3) {
        // For priorities 0-2, use the existing logic
        for(p = proc; p < &proc[NPROC]; p++) {
          acquire(&p->lock);
        ...
        // For priority 3 (lowest), implement round-robin
        int checked = 0;
        while(checked < NPROC) {
          last_proc_index = (last_proc_index + 1) % NPROC;
          p = &proc[last_proc_index];
          acquire(&p->lock);
